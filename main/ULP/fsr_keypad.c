...
volatile gpio_num_t keypadWritePins[X_SIZE] = {
    GPIO_NUM_21, GPIO_NUM_17, GPIO_NUM_1, GPIO_NUM_6,
    GPIO_NUM_12, GPIO_NUM_13, GPIO_NUM_14, GPIO_NUM_15
};

volatile adc_channel_t keypadReadAdcChannel[Y_SIZE] = {
...
static inline uint16_t sample_stable_reading(adc_channel_t channel) {
...
int main(void) {
    ...
    for (uint8_t x = 0; x < X_SIZE; ++x) {
        ...
        for (uint8_t y = 0; y < Y_SIZE; ++y) {
            result[x][y] = 0;
            stable_sample[x][y] = 0;
            for (uint8_t h = 0; h < HISTORY_SIZE; ++h) {
                raw_history[h][x][y] = 0;
            }
        }
    }
    ...
    while (true) {
        ...
        for (uint8_t x = 0; x < X_SIZE; ++x) {
            ...
            for (uint8_t y = 0; y < Y_SIZE; ++y) {
                uint16_t reading = sample_stable_reading(keypadReadAdcChannel[y]);
                stable_sample[x][y] = reading;
