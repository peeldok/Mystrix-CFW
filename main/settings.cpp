...
static std::atomic<RotationMode> rotation_mode{RotationMode::Deg0};
...
static void save_rotation(RotationMode rotation) {
    nvs_handle_t h;
    if (nvs_open("settings", NVS_READWRITE, &h) != ESP_OK) return;
    nvs_set_u8(h, "rotation", static_cast<uint8_t>(rotation));
    nvs_commit(h);
    nvs_close(h);
}
...
bool settings_init() {
    ...
    if (nvs_open("settings", NVS_READONLY, &h) == ESP_OK) {
        ...
        nvs_get_u8(h, "rotation", &saved_rotation);
        nvs_close(h);
    }
    ...
    if (saved_rotation > 3) saved_rotation = 0;
    rotation_mode.store(static_cast<RotationMode>(saved_rotation), std::memory_order_release);
...
RotationMode settings_rotation() {
    return rotation_mode.load(std::memory_order_acquire);
}
...
void settings_set_rotation(RotationMode rotation) {
    if (!ui_active.load(std::memory_order_acquire)) return;
    if (current_page.load(std::memory_order_acquire) != SettingsPage::Main) return;

    const uint8_t value = static_cast<uint8_t>(rotation);
    if (value > 3) return;

    rotation_mode.store(rotation, std::memory_order_release);
